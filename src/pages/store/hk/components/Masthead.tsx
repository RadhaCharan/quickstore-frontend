        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            onClick={() => whatsappShare(
              `🛒 Shop at *${store.storeName}*!\n${store.tagline || 'Fresh products delivered fast'}\n\n👉 ${window.location.origin}${base}`
            )}
            title="Share store on WhatsApp"
            style={{ background: '#25D366', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, color: '#fff', fontSize: 12, fontWeight: 600 }}
          >
